...
<div  contenteditable="true" style="-moz-border-radius: 4px;-khtml-border-radius: 4px;-webkit-border-radius: 4px;border-radius: 4px; color:#d7d7d7; font-size:16px; text-align:center; background:url(static/img/bgt.png) repeat ; padding:5px 0px; "><div style="float:left; padding-left:10px;">中国经济改革与发展研究院</div><div style="float:right; padding-right:10px;">  13-06-23</div><div style=" clear:both;"></div></div>
  ...
  <div class="xiangmu" style="-moz-border-radius: 6px;-khtml-border-radius: 6px;-webkit-border-radius: 6px;border-radius: 6px;   background:url(static/img/bgt.png) repeat ; padding:5px; margin-bottom:10px;">
    ...
    <div style=" text-align:center;  color:#d7d7d7;">
      <div style="float:left; width:113px;">提交请求 </div>
      <div style="float:left; width:180px; margin-left:34px;">返回验证码</div>
      <div style="float:left; width:180px;">提交用户名和密码信息</div>
...
<script type="text/javascript">
	...
	var status = {
		s:0,
		hository:new Array(8)
	}
	
	var startStep = function(i,bfb){
			i=i+1
			bfb =bfb*140/100;
			$(".bfb"+i+" img").animate({width:bfb+"px"},bfb*100)
		}
	
	var loadBegin = function(s){
			for(i=1;i<=s;i++){
	...
	var onMessage = function(data){
			...
			status.s = data.step
			status[data.step] = data
			loadBegin(data.step)
			startStep(data.step,88)
	}
